                ...
                <div class="card-header">
                    <div class="row">
                        ...
                        <div class="col-md-6 text-end">
                            <a href="{{route('room.create')}}" class="btn btn-sm bg-primary text-white">Add Room</a>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="table-responsive">
                            <table id="datatable-buttons" class="table table-striped table-bordered">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td data-priority="1">{{ $room->id }}</td>
                                        <td data-priority="1">{{$room->property->name}}</td>
                                        <td data-priority="1">XAF{{ $room->property->amount }}</td>
                                        <td data-priority="1">{{ $room->type }}</td>
                                        <td data-priority="1"> {{$room->property->city}}</td>
                                        <td data-priority="1" class="text-end">
                                            <form method="POST" action="{{route('activate-property')}}"
                                                class="active-deactive-form">
                                                @csrf
                                                <input name="property_id" value="{{$room->property->id}}" hidden/>
                                                <input name="current_status" value="{{$room->property->status}}" hidden/>
                                                <div class="form-check form-switch">
                                                    @if($room->property->status)
                                        ...
                                        <td data-priority="1" class="text-end">
                                            <form action="" method="Post">
                                                ...
                                                <a class="btn btn-primary" href="{{ route('room.edit',$room->id) }}"> <i class='bx bxs-pencil'></i></a>
                                                @csrf
                                                @method('DELETE')
                                ...
                                <div class="app-card app-card-doc shadow-sm  h-100">
                                    <div class="app-card-thumb-holder p-3">
                                        <div class="app-card-thumb">
                                            <img class="thumb-image" src="/uploads/images/property/house/room/{{json_decode($room->property->image)[0]}}" alt="">
                                        </div>
                                            <a class="app-card-link-mask" href="#file-link"></a>
                                    </div>
                                    <div class="app-card-body p-3 has-card-actions">
                                        
                                        <h4 class="app-doc-title truncate mb-0"><a href="#file-link">{{$room->property->name}}</a></h4>
                                        <div class="app-doc-meta">
                                            <ul class="list-unstyled mb-0">
                                                <li><span class="text-muted">Price:</span> {{$room->property->amount}}</li>
                                                <li><span class="text-muted">Type:</span> {{$room->type}}</li>
                                                <li><span class="text-muted">Location:</span> {{$room->property->city}}</li>
                                            </ul>
                                        </div><!--//app-doc-meta-->
                                        ...
                                        <div class="app-card-actions">
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu">
                                                    ...
                                                    <li>
                                                        <a class="dropdown-item" href="{{ route('room.edit',$room->id) }}"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pencil me-2" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
